...
namespace PracticeCapital_ByNikitaRasputin
{
    ...
    {
        public static async void BFONalog(List<string> inns, DataTable dataTable, CheckBox elementsState, CancellationToken token, ErrorStacker errorStacker)
        {
            ...
            for (int i = 0; i < inns.Count; i++)
            {
                ...
                {
                    errorStacker.AddError(inns[i], "BFONalog", ex);
                }
            }
            MessageBox.Show("Поиск завершен", "Конец операции", MessageBoxButtons.OK, MessageBoxIcon.Information);
            elementsState.Checked = true;
        }
        public static async void ClearBusiness(List<string> inns, DataTable dataTable, ChromiumWebBrowser browser, CheckBox elementsState, CancellationToken token, ErrorStacker errorStacker)
        {
            token.Register(() => { return; });
            if (!dataTable.Columns.Contains("Адрес")) dataTable.Columns.Add("Адрес", typeof(string));
            if (!dataTable.Columns.Contains("ОКВЭД")) dataTable.Columns.Add("ОКВЭД", typeof(string));
            ...
            for (int i = 0; i < inns.Count; i++)
            {
                ...
                {
                    errorStacker.AddError(inns[i], "ClearBusiness", ex);
                }
            }
            MessageBox.Show("Поиск завершен", "Конец операции", MessageBoxButtons.OK, MessageBoxIcon.Information);
            elementsState.Checked = true;
        ...
        public static async void FindOrg(List<string> inns, DataTable dataTable, ChromiumWebBrowser browser, CheckBox elementsState, CancellationToken token, ErrorStacker errorStacker)
        {
            token.Register(() => { return; });
            if (!dataTable.Columns.Contains("Адрес")) dataTable.Columns.Add("Адрес", typeof(string));
            if (!dataTable.Columns.Contains("ОКВЭД")) dataTable.Columns.Add("ОКВЭД", typeof(string));
            ...
            for (int i = 0; i < inns.Count; i++)
            {
                ...
                {
                    errorStacker.AddError(inns[i], "FindOrg", ex);
                }
            }
            MessageBox.Show("Поиск завершен", "Конец операции", MessageBoxButtons.OK, MessageBoxIcon.Information);
            elementsState.Checked = true;
        }
        public static async void ListOrg(List<string> inns, DataTable dataTable, ChromiumWebBrowser browser, CheckBox elementsState, CancellationToken token, ErrorStacker errorStacker)
        {
            token.Register(() => { return; });
            if (!dataTable.Columns.Contains("Адрес")) dataTable.Columns.Add("Адрес", typeof(string));
            if (!dataTable.Columns.Contains("ОКВЭД")) dataTable.Columns.Add("ОКВЭД", typeof(string));
            ...
            for (int i = 0; i < inns.Count; i++)
            {
                ...
                {
                    errorStacker.AddError(inns[i], "ListOrg", ex);
                }
            }
            MessageBox.Show("Поиск завершен", "Конец операции", MessageBoxButtons.OK, MessageBoxIcon.Information);
            elementsState.Checked = true;
        }
        public static async void Arbitr(List<string> inns, DataTable dataTable, ChromiumWebBrowser browser, CheckBox elementsState, CancellationToken token, ErrorStacker errorStacker)
        {
            token.Register(() => { return; });
            if (!dataTable.Columns.Contains("Кол-во судебных дел")) dataTable.Columns.Add("Кол-во судебных дел", typeof(string));
            ...
            for (int i = 0; i < inns.Count; i++)
            {
                ...
                {
                    errorStacker.AddError(inns[i], "Arbitr", ex);
                }
            }
            MessageBox.Show("Поиск завершен", "Конец операции", MessageBoxButtons.OK, MessageBoxIcon.Information);
            elementsState.Checked = true;
        }
        public static async void ForFairBussiness(List<string> inns, DataTable dataTable, ChromiumWebBrowser browser, CheckBox elementsState, CancellationToken token, ErrorStacker errorStacker)
        {
            if (!dataTable.Columns.Contains("Адрес")) dataTable.Columns.Add("Адрес", typeof(string));
            if (!dataTable.Columns.Contains("ОКВЭД")) dataTable.Columns.Add("ОКВЭД", typeof(string));
            if (!dataTable.Columns.Contains("Дата регистрации")) dataTable.Columns.Add("Дата регистрации", typeof(string));
            ...
            for (int i = 0; i < inns.Count; i++)
            {
                ...
                {
                    errorStacker.AddError(inns[i], "ForFairBusiness", ex);
                }
            }
            MessageBox.Show("Поиск завершен", "Конец операции", MessageBoxButtons.OK, MessageBoxIcon.Information);
            elementsState.Checked = true;
